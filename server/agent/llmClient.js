...
function resolveApiKey(provider, explicitKeys = {}) {
  ...
  // Highest priority: passed-in explicit keys, e.g. { deepseek: "sk-..." }
  if (explicitKeys[provider]) {
    return explicitKeys[provider];
  }

  // Provider-specific env var (e.g., DEEPSEEK_API_KEY, KIMI_API_KEY, OPENAI_API_KEY)
...
async function callLLM({
  ...
  try {
    ...
    // Log error with provider and model context
    ...
    if (error.response) {
      ...
      throw new Error(`LLM API error (${status}): ${JSON.stringify(data)}`);
