...
// Orchestrates agent execution: mode resolution, LLM calls, tool execution
...
async function runAgent(request) {
  ...
  console.log(`[Orchestrator] Running agent in mode "${modeName}" with provider "${provider}"`);
  ...
  // Step 3: Process result (in future, this could parse tool calls from LLM response)
  const result = {
